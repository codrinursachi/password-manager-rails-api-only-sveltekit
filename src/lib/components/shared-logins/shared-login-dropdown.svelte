<script lang="ts">
	...
	let sharedLoginMutation = createMutation({
	...
	$effect(() => {
		byMe;
		sharedLoginMutation = createMutation({
			mutationKey: ['sharedLogins', 'delete'],
			mutationFn: async (loginId: string) => {
				await mutateSharedLogin(null, loginId);
			},
...
<DropdownMenu.Root open={dropdownOpen} onOpenChange={(isOpen) => (dropdownOpen = isOpen)}>
	...
	<DropdownMenu.Content side="right" align="start">
		<DropdownMenu.Item>
			<button onclick={() => navigator.clipboard.writeText(login.login_name)}>
				Copy username
			</button>
		</DropdownMenu.Item>
		<DropdownMenu.Item>
			<button
				onclick={async () =>
					byMe
						? navigator.clipboard.writeText(await getPasswordSharedByMe(login.login_id.toString()))
						: navigator.clipboard.writeText(await getPasswordSharedWithMe(login.login_password))}
			>
				Copy password
			</button>
		</DropdownMenu.Item>
		{#if login.file}
			<DropdownMenu.Item>
				<a href={'http://127.0.0.1:3000' + login.file} download target="_self"> Download file </a>
			</DropdownMenu.Item>
		{/if}
		<form
			onsubmit={(event) => {
