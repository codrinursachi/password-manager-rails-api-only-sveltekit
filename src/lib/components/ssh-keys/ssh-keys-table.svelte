<script lang="ts">
	...
	const pendingSSHKeysAdd = useMutationState({
		...
		select: (mutation) =>
			(mutation.state.variables as { formData: FormData }).formData.get('sshkey[name]')!.toString()
	});
...
<Table.Root class="table-fixed">
	...
	<Table.Body>
		{#each data as sshKey (sshKey.id)}
			{@const pendingDelete = $pendingSSHKeysDelete.find((key) => key === sshKey.id)}
			{@const pendingEdit = $pendingSSHKeysEdit.find((key) => key.id === sshKey.id.toString())}
			<Table.Row class={pendingEdit ? 'text-green-500' : pendingDelete ? 'text-red-500' : ''}>
				<Table.Cell>
					<button
						...
						class="w-full text-left">{pendingEdit?.name ?? sshKey.name}</button
					>
				</Table.Cell>
				<Table.Cell>
					<Dialog.Root>
						...
						<Dialog.Content>
							<Dialog.Header>
								<Dialog.Title>Are you sure you want to delete key pair?</Dialog.Title>
							</Dialog.Header>
							<Dialog.Description class="hidden">Delete ssh key.</Dialog.Description>
							<Dialog.Footer>
								<Dialog.Close>
									{#snippet child({ props })}
